...
/**
 * @param {{ address: string; title: string; description?: string }} item
 */
 ...
 *
 * @returns {Array<{ title: string; address: string; description?: string }>}
 */
function parseXML(input, feedTitle, feedType) {
  ...
  for (const match of itemMatches) {
    ...
    const itemDescription = getItemDescription(matchText);
...
function getItemDescription(item) {
  const descriptionMatcher = /\<description\>(.*?)\<\/description\>/;
  const descriptionMatch = item.match(descriptionMatcher);
